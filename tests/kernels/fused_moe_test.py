...
try:
    ...
    # Fallback gracefully if already initialized or in a single-host environment
    pass
...
class MoEKernelTest(jtu.JaxTestCase):
    ...
    def _test_moe(
        ...
        per_shard = intermediate_size
        if use_ep:
            aligned = (-(per_shard // -256)) * 256
        else:
            per_shard //= num_cores
            aligned = (-(per_shard // -256)) * 256
            aligned *= num_cores
        intermediate_size = aligned
        ...
        if use_ep:
            ...
            if w_dtype is not None:
                ...
                w2_scale = jax.device_put(
                    w2_scale,
                    NamedSharding(mesh, P(None, None)) if w2_scale.shape[1]
                    == 1 else NamedSharding(mesh, P(None, "model")),
                )
            if has_bias:
                b1 = jax.device_put(
    ...
    def test_benchmark_qwen_coder_480b(self, num_tokens, w_dtype, block_size,
                                       use_ep, active_expert_ratio):
        num_experts = 160
        top_k = 8
    ...
    def test_benchmark_mistral(self, num_tokens, w_dtype, block_size, use_ep,
                               active_expert_ratio):
        num_experts = 128
        top_k = 4
